...
def glossy_columns(src, out, target=(1200,900), n=40, reg=(0.11,0.13,0.87,0.90), seed=7, alpha=0.62, blur=2.5, fmt="WEBP", quality=90):
    """Reusable 'sheared glass slats' effect: a semi-transparent glass pane sliced
    into vertical slats, each vertically SHEARING the scene (offset ramps across the
    slat width, tooth height ~ slat width) so edges become a fine ~45deg sawtooth
    comb (not spikes). A soft Gaussian blur gives the dreamy shimmer of the ref."""
    ...
    for i in range(n):
        ...
        xf=(np.arange(w)+0.5)/w-0.5; off=base+ramp*xf
    ...
    if blur>0:
        paneimg=Image.fromarray(np.clip(out_a[y0:y1,x0:x1,:],0,255).astype("uint8")).filter(ImageFilter.GaussianBlur(blur))
        out_a[y0:y1,x0:x1,:]=np.asarray(paneimg).astype(np.float32)
